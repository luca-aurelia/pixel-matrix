...
export default class PixelMatrix {
  ...
  constructor (width, height, imageData) {
    ...
    if (pixelsLength !== pixels.length) {
      throw new Error(
        `Expected pixels to have length ${pixelsLength} (width * height * 4) but got ${pixels.length} instead. (We multiply by 4 because we have 4 channels: red, green, blue, and alpha.)`
      )
    }
  ...
  get (x, y) {
    return [
      this.getColor(x, y, 'red'),
      this.getColor(x, y, 'green'),
      this.getColor(x, y, 'blue'),
      this.getColor(x, y, 'alpha')
    ]
  }
  ...
  getColor (x, y, channelName) {
    if (!this.contains(x, y)) {
      throw new Error(
        `This pixel matrix doesn't contain the point (${x}, ${y}).`
      )
    }
  ...
  getIndex (x, y) {
    if (!this.contains(x, y)) {
      throw new Error(
        `Expected x and y to be less than or equal to (${this.width}, ${this.height}) but they were actually (${x}, ${y}).`
      )
    }
  ...
  drawToContext (context) {
    if (
      ...
      throw new Error(
        `Expected canvas shape and PixelMatrix shape to be the same, but canvas shape was [${context.canvas.width}, ${context.canvas.height}] and PixelMatrix shape was [${this.width}, ${this.height}].`
      )
    }
